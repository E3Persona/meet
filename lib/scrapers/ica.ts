...
function findEmailAnchor($: cheerio.CheerioAPI): ReturnType<cheerio.CheerioAPI> | null {
...
function decodeEmailFromAnchor($: cheerio.CheerioAPI, $a: ReturnType<cheerio.CheerioAPI>): string | null {
...
async function fetchListingPage(
  ...
  $('a[href^="/event-"]').each((_, el) => {
    ...
    const badges = $el.find('span[data-slot="badge"]')
      .map((_, b) => $(b).text().trim())
      .get();
    ...
    const infoItems = $el.find('div.text-muted-foreground > div')
      .map((_, d) => $(d).text().trim())
      .get()
      .filter(Boolean);
...
async function scrapeDetailPage(
  ...
  const organizerEmail = $emailAnchor ? decodeEmailFromAnchor($, $emailAnchor) : null;
  ...
  if ($emailAnchor) {
    ...
    if (emailIdx >= 2) {
      organizerOrg = $(siblings[emailIdx - 2]).text().trim() || null;
      organizerName = $(siblings[emailIdx - 1]).text().trim() || null;
    } else if (emailIdx === 1) {
      organizerName = $(siblings[emailIdx - 1]).text().trim() || null;
    }
  }
        ...
        while (pageNum <= Math.min(totalPages, maxPages)) {
          ...
          for (const card of cards) {
            ...
            results.push({
              ...
              venueCountry: detail.venueCountry ?? card.country ?? "United States",
              contacts: [
                {
